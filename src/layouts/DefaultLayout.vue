  ...
  <q-layout view="hHh Lpr fFf"> 
     <q-header :elevated="isElevated" :class="headerClass" height-hint="100">
      ...
      <q-toolbar class="desktop-only">
        ...
        <q-btn flat dense v-for="(item,i) in menuList" :key="i"
        :to="item.link" :label="item.title"
      ...
      <q-toolbar class="mobile-only">
          ...
          <q-btn flat @click="drawerRight = !drawerRight" round dense icon="eva-menu-outline" />
        </q-toolbar>
...
export default {
  ...
  data () {
    return {
      ...
      {title: 'About', link: '/about', icon: 'eva-smiling-face-outline'},
      {title: 'Skills', link: '/#skills', icon: 'eva-bulb-outline'},
      {title: 'Projects', link: '/#project', icon: 'eva-browser-outline'},
      {title: 'Contact', link: '#contact', icon: 'eva-message-square-outline'}
    ]
    }
  },
  methods: {
    ...
    handleScroll(event) {      
      if(window.scrollY > 200){
        ...
        this.headerClass = "text-primary bg-accent"
      }else{
